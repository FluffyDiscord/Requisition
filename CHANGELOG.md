...
- Defragment no longer freezes the game on a large drive bay — a full 40-disk bay went from a third of a second to a fifth of a frame, and a bay packed with bulk materials from roughly seven tenths of a second to the same
- Defragment: a repeated disk in a multiplayer defrag request can no longer make a disk donate to itself
